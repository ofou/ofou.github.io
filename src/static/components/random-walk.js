(function () {
  Fig.register("random-walk", function (el, opts) {
    ...
    Fig.frame(cv);
    const ctx = cv.ctx;
    let steps = 400, seedBase = 1;
    function mulberry32(a) { return () => { a |= 0; a = a + 0x6D2B79F5 | 0; let t = Math.imul(a ^ a >>> 15, 1 | a); t = t + Math.imul(t ^ t >>> 7, 61 | t) ^ t; return ((t ^ t >>> 14) >>> 0) / 4294967296; }; }
    Fig.controls(el, [{ key: "steps", label: "Steps", min: 50, max: 2000, step: 25, value: steps }],
      (k, v) => { steps = Math.round(v); walks(); draw(); });
    ...
    function walks() {
      ...
      for (let n = 0; n < 8; n++) {
        ...
        let x = cv.w / 2, y = cv.h / 2;
        for (let s = 0; s < steps; s++) {
          x += (rnd() - 0.5) * 6; y += (rnd() - 0.5) * 6;

(function () {
  Fig.register("iso-cube", function (el, opts) {
    ...
    Fig.controls(el, [
      ...
      { key: "pitch", label: "Pitch", min: 0, max: 90, step: 1, value: pitch },
    ], (k, v) => { if (k === "yaw") yaw = v; if (k === "pitch") pitch = v; draw(); });
    ...
    function project(v) {
      ...
      let y1 = y * Math.cos(rp) - z1 * Math.sin(rp), z2 = y * Math.sin(rp) + z1 * Math.cos(rp);

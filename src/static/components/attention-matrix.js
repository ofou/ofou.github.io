(function () {
  Fig.register("attention-matrix", function (el, opts) {
    ...
    let T = 1, hover = null;
    Fig.controls(el, [{ key: "T", label: "Temperature", min: 0.1, max: 2, step: 0.05, value: T }],
      (k, v) => { T = v; draw(); });
    Fig.caption(el, "hover a cell · softmax(q·k / T)");
    function weights() {
      const seedVec = w => { let h = 2166136261; for (const ch of w) { h ^= ch.charCodeAt(0); h = Math.imul(h, 16777619); } return [h % 97 / 97, (h >> 5) % 89 / 89]; };
      const q = toks.map(seedVec), k = toks.map(t => seedVec(t.split("").reverse().join("")));
      return q.map(qv => {
    ...
    const draw = () => {
      ...
      grid.forEach((row, i) => row.forEach((w, j) => {
    ...
    cv.el.addEventListener("pointermove", e => {
      ...
      readout.textContent = hover ? `${toks[hover[1]]} → ${toks[hover[2]]} : ${hover[0].toFixed(3)}` : "hover a cell";
      draw();
    });

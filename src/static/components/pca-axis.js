(function () {
  Fig.register("pca-axis", function (el, opts) {
    ...
    for (let i = 0; i < 120; i++) {
      const u = rnd(), v = rnd();
      cloud.push({ x: 0.15 + u * 0.6 + v * 0.12, y: 0.15 + u * 0.18 + v * 0.55 });
    }
    let theta = 30;
    Fig.controls(el, [{ key: "theta", label: "Axis θ", min: 0, max: 180, step: 1, value: theta }],
      (k, v) => { theta = v; draw(); });
    const readout = document.createElement("p");
    readout.className = "meta"; readout.style.margin = "0.4rem 0 0";
    el.appendChild(readout);
    const CX = cv.w / 2, CY = cv.h / 2, R = Math.min(cv.w, cv.h) * 0.42;
    const dir = () => [Math.cos(theta * Math.PI / 180), -Math.sin(theta * Math.PI / 180)];
    function project(p, d) {
      const px = (p.x - 0.5) * 2, py = -(p.y - 0.5) * 2;
    ...
    const draw = () => {
      ...
      for (const p of cloud) {
        ...
        const gx = CX + d[0] * pr.along * R, gy = CY + d[1] * pr.along * R;
        ctx.strokeStyle = P.rule;
        ctx.beginPath(); ctx.moveTo(CX + p.x * 60 - 30, CY + p.y * 90 - 45); ctx.lineTo(gx, gy); ctx.stroke();

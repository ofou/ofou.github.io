(function () {
  Fig.register("q-learning", function (el, opts) {
    ...
    panel.style.cssText =
      "flex:0 0 150px;font:11px ui-monospace,Menlo,monospace;color:var(--mute)";
    ...
    const isTerminal = (c, r) =>
      (c === GOAL[0] && r === GOAL[1]) ||
      (c === PENALTY[0] && r === PENALTY[1]);
    ...
    Fig.caption(
      el,
      "Q-learning on a 4\u00d73 gridworld: \u03b5-greedy walks, shaded by max Q",
    );
    ...
    function draw() {
        ...
        for (let c = 0; c < COLS; c++) {
          ...
          if (isWall(c, r)) {
            ...
            for (let d = -size; d < size * 2; d += 6) {
              ctx.beginPath();
              ctx.moveTo(x + d, y + size + 1);
              ctx.lineTo(x + d + size + 2, y - 1);
              ctx.stroke();
            }
            ctx.restore();
            continue;
      ...
      // agent dot
      const ax = ox + agent[0] * size + size / 2,
        ay = oy + agent[1] * size + size / 2;
      ctx.beginPath();
      ctx.arc(ax, ay, Math.max(4, size * 0.14), 0, Math.PI * 2);

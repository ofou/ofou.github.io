...
(function () {
  Fig.register("pdf-router", function (el) {
    ...
    const SCEN = [
      { id: "native",   chip: "native",       title: "engine: native \u00B7 pinned venice" },
      { id: "native400", chip: "native \u00B7 400", title: "engine: native \u00B7 pinned alibaba" },
      { id: "cf",       chip: "cloudflare",   title: "engine: pdf-text \u00B7 cloudflare-ai" },
      { id: "ocr",      chip: "mistral-ocr",  title: "engine: mistral-ocr" },
      { id: "omitted",  chip: "omitted",      title: "engine omitted \u00B7 silent default" },
      { id: "images",   chip: "images",       title: "images route \u00B7 pdftoppm \u2192 png" },
    ];
    const LEG = 800, HOLD = 2000;               // ms per leg, ms result hold
    let scen = 0, phase = 0, autoCycle = true;
    let phaseStart = performance.now(), doneAt = 0;
    let chips = [];

    function geom() {
      const w = cv.w, h = cv.h;
      const colW = Math.min(112, Math.max(80, w * 0.22));
      const provX = w - colW, dotX = provX - 8, x0 = 12;
    ...
    function ctrlPt(a, b) {
      ...
      return [(a[0] + b[0]) / 2 - dy / L * k, (a[1] + b[1]) / 2 + dx / L * k];
    }
    const qPoint = (a, c, b, u) => {
      const v = 1 - u;
      return [v * v * a[0] + 2 * v * u * c[0] + u * u * b[0],
    ...
    function edge(a, b, col, aw) {
      ...
      ctx.quadraticCurveTo(c[0], c[1], b[0], b[1]); ctx.stroke();
    ...
    function plan(g, s) {
      const pdfR = rightOf(g.pdf), engL = leftOf(g.engine), engR = rightOf(g.engine);
      const parL = leftOf(g.parser), parR = rightOf(g.parser);
      const ven = [g.dotX + 2, g.provY(VENICE)], ali = [g.dotX + 2, g.provY(3)];
      const renL = leftOf(g.render), renR = rightOf(g.render);
      switch (s) {
        case 0: return { legs: [[pdfR, engL], [engR, ven]],
          ok: true, target: VENICE, note: "raw PDF \u2192 venice \u00B7 7 s \u00B7 $0.006" };
        case 1: return { legs: [[pdfR, engL], [engR, ali]],
          ok: false, target: 3, note: "other 7 \u2192 400 no file input" };
        case 2: return { legs: [[pdfR, engL], [engR, parL], [parR, ven]],
          ok: true, target: -2, note: "parsed \u2192 markdown \u00B7 FREE \u00B7 any provider", parse: true, fanout: true };
        case 3: return { legs: [[pdfR, engL], [engR, parL], [parR, ven]],
          ok: true, target: -2, note: "OCR \u00B7 $2 / 1000 pages \u00B7 6\u00D7 cost", parse: true, paid: true, fanout: true };
        case 4: return { legs: [[pdfR, engL], [engR, parL], [parR, ven]],
          ok: true, target: -2, note: "silent default \u2192 PAID mistral-ocr", parse: true, paid: true, fanout: true };
        case 5: return { legs: [[pdfR, renL], [renR, [g.dotX + 2, g.provY(0)]]],
          ok: true, target: -2, note: "918 tok/page \u00B7 all 8 providers \u00B7 pinned or not", images: true, fanout: true };
      }
    }

    function packet(a, b, u, col, r) {
      const c = ctrlPt(a, b);
      for (let k = 6; k >= 1; k--) {           // fading trail behind the head
        const uu = ease(u) - k * 0.055;
        if (uu <= 0) continue;
        const q = qPoint(a, c, b, uu);
        ctx.globalAlpha = (1 - k / 7) * 0.35;
        ctx.beginPath(); ctx.arc(q[0], q[1], Math.max(0.6, r - k * 0.45), 0, Math.PI * 2);
    ...
    const draw = () => {
      ...
      box(g.parser, p.paid ? "OCR  $" : "parse\u2192md", !!p.parse && phase >= 1);
      ...
      for (let i = 0; i < PROVIDERS.length; i++) {
        const y = g.provY(i);
        let c = P.mute, mark = "", hit = false;
        if (done) {
          if (p.target === -2) { c = P.accent; mark = " \u2713"; hit = true; }
          else if (i === p.target) { c = P.accent; mark = p.ok ? " \u2713" : " \u2717 400"; hit = true; }
        }
        if (i === VENICE) { ctx.fillStyle = P.ink; ctx.fillText("\u25C6", g.provX - 12, y); }
        ctx.fillStyle = c;
        ctx.fillText(fitText(PROVIDERS[i] + mark, g.colW - 6), g.provX + 2, y);
        ctx.fillStyle = done && mark.includes("\u2717") ? c : (done && mark ? P.accent : P.mute);
        ctx.beginPath(); ctx.arc(g.dotX + 2, y, 2.5, 0, Math.PI * 2); ctx.fill();
        if (hit && pulse > 0) {                // outcome pulse: expanding ring
          ctx.globalAlpha = 1 - pd / 0.7;
          ctx.strokeStyle = P.accent; ctx.lineWidth = 1.2;
          ctx.beginPath(); ctx.arc(g.dotX + 2, y, 3 + pulse * 6, 0, Math.PI * 2); ctx.stroke();
          ctx.globalAlpha = 1;
        }
      }

      // scenario caption + outcome note
      ctx.font = MONO10; ctx.textAlign = "left";
      ctx.fillStyle = P.ink;
      ...
      if (done) {
        ...
        if (pulse > 0) {                       // brief underline swell under the note
          ctx.strokeStyle = p.ok ? P.accent : P.mute; ctx.lineWidth = 1;
          const nw = Math.min(ctx.measureText(p.note).width, g.w - 28) * pulse;
          ctx.beginPath(); ctx.moveTo(14, g.h - 5); ctx.lineTo(14 + nw, g.h - 5); ctx.stroke();
        }
        ctx.globalAlpha = 1;
      }
      ...
      if (!done) {
        ...
        if (p.fanout && last) {                // fan out to ALL 8 providers
        ...
        phase = 0; phaseStart = now; doneAt = 0;
    ...
    SCEN.forEach((s, i) => {
      const b = Fig.chip(s.chip, () => {
        scen = i; phase = 0; phaseStart = performance.now(); doneAt = 0;
        autoCycle = false;
        syncChips();
        cv.redraw && cv.redraw();
      });
    ...
    Fig.caption(el, "packet = request \u00B7 chips pin path \u00B7 \u25C6 = only native PDF host \u00B7 bottom = outcome");

(function () {
  Fig.register("policy-gradient", function (el, opts) {
    ...
    Fig.frame(cv);
    const ctx = cv.ctx;
    let pi = 0.5, pi0 = 0.5, lr = 0.1, t = 0;
    const history = [];
    function mulberry32(a) { return () => { a |= 0; a = a + 0x6D2B79F5 | 0; let t = Math.imul(a ^ a >>> 15, 1 | a); t = t + Math.imul(t ^ t >>> 7, 61 | t) ^ t; return ((t ^ t >>> 14) >>> 0) / 4294967296; }; }
    let rnd = mulberry32(99);
    Fig.controls(el, [
      { key: "pi0", label: "Initial π", min: 0.05, max: 0.95, step: 0.05, value: pi0 },
      { key: "lr", label: "Learning rate", min: 0.01, max: 0.3, step: 0.01, value: lr },
    ], (k, v) => {
      if (k === "pi0") { pi = pi0 = v; t = 0; history.length = 0; rnd = mulberry32(99); }
      if (k === "lr") lr = v;
      draw();
    });
    const reset = Fig.chip("↺ Reset", () => { pi = pi0; t = 0; history.length = 0; rnd = mulberry32(99); draw(); });
    ...
    const draw = () => {
      ...
      ctx.fillStyle = pi > 0.5 ? P.paper : P.ink; ctx.textAlign = "center";
      if (1 - pi > 0.12) ctx.fillText("← left " + (1 - pi).toFixed(2), 12 + bw * (1 - pi) / 2, 45);
      ctx.fillStyle = pi > 0.5 ? P.paper : P.accent;
      if (pi > 0.12) ctx.fillText("right " + pi.toFixed(2) + " →", 12 + bw * (1 - pi) + bw * pi / 2, 45);
      ...
      if (history.length > 1) {
        ...
        ctx.beginPath();
        history.forEach((v, i) => {
          const x = 12 + (i / 200) * bw, y = 96 + (1 - v) * (cv.h - 130);
    ...
    cv.redraw = draw;
    Fig.onScheme(draw);
    Fig.animate(cv, () => { tick(); if (t % 3 === 0) draw(); });
  });
})();

(function () {
  Fig.register("reward-shaping", function (el) {
    ...
    const GOAL = 20, START = 0, CAP = 900;   // step cap = failed episode
    ...
    let eps = 0.15, speed = 40, rng;

    const makeAgent = () => ({ pos: START, p: [0, 0], trace: [], steps: 0 });
    const mkVariant = (name, shaped) => ({
      name, shaped, agent: makeAgent(), hist: [], done: 0, bAvg: 400,
    });
    let VA, VB;
    const reset = () => {
      rng = mulberry32(1234);
    ...
    function step(v) {
      const g = v.agent;
      let a;
      if (g.p[1] > g.p[0] + DELTA) a = 1;        // decisively prefer right
      else if (g.p[0] > g.p[1] + DELTA) a = 0;   // decisively prefer left
      ...
      g.p[0] *= 1 - LAM; g.p[1] *= 1 - LAM;

      if (g.pos >= GOAL || g.steps >= CAP) {
        const success = g.pos >= GOAL;
        const adv = Math.max(-1, Math.min(1, (v.bAvg - g.steps) / v.bAvg)) / g.trace.length;
    ...
    // ── controls ─────────────────────────────────────────────────────────
    Fig.controls(el, [
      { key: "eps", label: "Explore ε", min: 0.05, max: 0.5, step: 0.05, value: eps },
      { key: "speed", label: "Speed", min: 1, max: 120, step: 1, value: speed },
    ], (k, v) => { if (k === "eps") eps = v; else speed = v; });

    const btnRow = document.createElement("div");
    btnRow.style.cssText = "display:flex;gap:0.5rem;margin-top:0.5rem";
    btnRow.appendChild(Fig.chip("↺ Reset", () => { reset(); draw(); }));
    el.appendChild(btnRow);
    Fig.caption(el, "same seeded corridor 0→20 · sparse: +1 only at goal · shaped: + (newDist−oldDist) per step");

    // ── layout ───────────────────────────────────────────────────────────
    const TOP = 8, GAP = 18, PH = (cv.h - TOP - GAP) / 2; // panel height
    const LX = 40, RX = 12;

    const drawVariant = (v, y0, colDot, colBar) => {
      const P = Fig.palette();
      const hw = cv.w - LX - RX;
      const hy = y0 + 18, hh = 108;
      ...
      const avg = recent.length
        ? Math.round(recent.reduce((s, x) => s + x, 0) / recent.length) : null;
      ctx.fillText(
        v.name + "  ·  EPISODES " + v.done +
        "  ·  AVG " + (avg == null ? "—" : avg) +
        "  ·  STEPS " + v.agent.steps +
        "  ·  POS " + v.agent.pos,
        LX, y0 + 11);
      ...
      // history plot
      const pitch = 3, K = Math.max(1, Math.floor(hw / pitch));
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(LX, hy + hh + 0.5); ctx.lineTo(LX + hw, hy + hh + 0.5);
      ctx.stroke();
      ctx.fillStyle = P.mute;
      ...
      shown.forEach((s, i) => {
        const x = LX + i * pitch;
        const bh = (s / m) * hh;
        if (s >= CAP) { // failed episode: outline only
          ctx.strokeStyle = colBar; ctx.globalAlpha = 0.45;
      ...
      ctx.strokeStyle = P.rule;
      ctx.beginPath(); ctx.moveTo(LX, cy + 0.5); ctx.lineTo(LX + hw, cy + 0.5); ctx.stroke();
      ctx.font = "9px ui-monospace, Menlo, monospace";
      ctx.fillStyle = P.mute;
      ...
      ctx.beginPath(); ctx.arc(X(v.agent.pos), cy - 4, 4.5, 0, 7); ctx.fill();
    };
    ...
    Fig.animate(cv, () => {
      ...
      for (let s = 0; s < n; s++) { step(VA); step(VB); }
      draw();
    });
  });
})();

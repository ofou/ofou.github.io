(function () {
  Fig.register("entropy", function (el, opts) {
    ...
    Fig.controls(
      el,
      [{ key: "p", label: "p", min: 0.01, max: 0.99, step: 0.01, value: p }],
      (k, v) => {
        p = v;
        draw();
      },
    ...
    const draw = () => {
      ...
      ctx.stroke();
      const dotX = curveX + p * curveW,
        dotY = 24 + curveH - H(p) * curveH;
      ctx.fillStyle = P.accent;
      ctx.beginPath();
      ctx.arc(dotX, dotY, 4.5, 0, 7);
      ctx.fill();
      ctx.font = "12px ui-monospace, Menlo, monospace";
      ctx.fillStyle = P.ink;
      ctx.fillText(
        "H = " + H(p).toFixed(3) + " bits",
        Math.min(dotX + 8, cv.w - 92),
        dotY - 8,
      );
    };
    draw();

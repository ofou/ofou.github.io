(function () {
  Fig.register("normal-curve", function (el, opts) {
    ...
    const draw = () => {
      ...
      ctx.lineTo(X(mu + sigma), cv.h - 14);
      ctx.closePath();
      ctx.fill();
      ctx.strokeStyle = P.rule;
      [mu - 2 * sigma, mu - sigma, mu, mu + sigma, mu + 2 * sigma].forEach(
        (t) => {
          ctx.beginPath();
          ctx.moveTo(X(t), cv.h - 14);
          ctx.lineTo(X(t), 16);
          ctx.stroke();
        },
      );
      ...
      [
        ...
        [mu - sigma, "−1σ"],
        [mu + sigma, "+1σ"],
      ].forEach(([t, l]) => ctx.fillText(l, X(t), cv.h - 2));

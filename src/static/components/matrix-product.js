(function () {
  Fig.register("matrix-product", function (el, opts) {
    ...
    const draw = () => {
      ...
      const order = [];
      for (let i = 0; i < 2; i++) for (let j = 0; j < 2; j++) for (let k = 0; k < 3; k++) order.push([i, j, k]);
      const [hi, hj, hk] = [order[step % order.length][0], order[step % order.length][1], order[step % order.length][2]];
      const cell = 34, gx = 24, gy = 30;
      ...
      function numbers(x, y, m, color) {
        ...
        m.forEach((row, i) => row.forEach((v, j) => ctx.fillText(String(v), x + j * cell + cell / 2 - 2, y + i * cell + cell / 2 + 4)));
        ctx.textAlign = "left";
      }
      grid(gx, gy, 2, 3, "row"); numbers(gx + 4, gy + 4, A);
      grid(gx + 3 * cell + 26, gy, 3, 2, "col"); numbers(gx + 3 * cell + 30, gy + 4, B);

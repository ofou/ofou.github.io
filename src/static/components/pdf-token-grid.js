...
(function () {
  Fig.register("pdf-token-grid", function (el) {
    ...
    const FACTOR = 32, MAX_PX = 16777216, ALI_PX = 2621440;
    ...
    const DMIN = 50, DMAX = 600, EASE_MS = 380;
    ...
    function smartResize(hPx, wPx, budget) {
      let hb = Math.round(hPx / FACTOR) * FACTOR, wb = Math.round(wPx / FACTOR) * FACTOR;
      if (hb * wb > budget) {
    ...
    const draw = () => {
      ...
      const wPx = Math.round(PAGE_IN[0] * dpi), hPx = Math.round(PAGE_IN[1] * dpi);
      const [hb, wb] = smartResize(hPx, wPx, MAX_PX);
      const [ha, wa] = smartResize(hPx, wPx, ALI_PX);
      const tok = tokens(hb, wb), tokA = tokens(ha, wa);
      const hs = Math.round(hPx / FACTOR) * FACTOR, ws = Math.round(wPx / FACTOR) * FACTOR;
      ...
      ctx.beginPath(); ctx.rect(px, py, pageW, pageH); ctx.fill(); ctx.stroke();

      // fake text lines — crispness degrades with the lost pixel fraction
      const nL = 15, mx = pageW * 0.09, lh = pageH / (nL + 2);
      ctx.save();
      if (lost > 0 && "filter" in ctx) ctx.filter = "blur(" + (lost * 2.4).toFixed(2) + "px)";
      ctx.globalAlpha = 1 - 0.55 * lost;
      ctx.fillStyle = P.ink;
      for (let i = 0; i < nL; i++) {
        const wf = (i % 6 === 5) ? 0.45 : 0.6 + 0.38 * frac(Math.sin((i + 1) * 12.9898) * 43758.5453);
        ctx.fillRect(px + mx, py + lh * (i + 1.5), (pageW - 2 * mx) * wf, Math.max(1, lh * 0.32));
      }
      ctx.restore();

      // token grid (cap drawn cells for perf; density is real ratio)
      const cols = wb / FACTOR, rows = hb / FACTOR;
      const step = Math.max(1, Math.ceil(cols / 60));
      ctx.strokeStyle = P.rule;
      ctx.lineWidth = 0.5;
      for (let c = step; c < cols; c += step) {
        const x = px + (c / cols) * pageW;
        ctx.beginPath(); ctx.moveTo(x, py); ctx.lineTo(x, py + pageH); ctx.stroke();
      }
      for (let r = step; r < rows; r += step) {
        const y = py + (r / rows) * pageH;
        ctx.beginPath(); ctx.moveTo(px, y); ctx.lineTo(px + pageW, y); ctx.stroke();
      }
      ...
      if (aliClipped) {
        ...
        const bx = px + pageW * (1 - fr) / 2, by = py + pageH * (1 - fr) / 2;
        ctx.save();
      ...
      const line = (txt, c) => { ctx.fillStyle = c || P.ink; ctx.fillText(txt, rx, y); y += lhR; };
      line("render   " + wPx + "\u00D7" + hPx + " px", P.mute);
      line("snapped  " + wb + "\u00D7" + hb + " px" + (clipped ? "  \u2193 downscaled" : ""), clipped ? P.mute : P.ink);
      ...
      if (aliClipped) {
        line("alibaba would bill " + tokA.toLocaleString() + " \u2192 $" + costA13.toFixed(4), P.mute);
      }
      y += 3;
      line(dpi > CEIL_DPI ? "over 16.7M px\u00B2 budget \u2192 resized" : "fits budget (ceiling: " + CEIL_DPI + " dpi)", P.mute);
      if (!aliClipped) line("alibaba cap: not hit yet", P.mute);

      ctx.fillStyle = P.ink;
      ctx.fillText("Letter @ " + dpiR + " dpi \u2192 smart_resize \u2192 tokens", px, 8);

      // dpi axis with notable tick marks
      const ax0 = px, ax1 = w - 16, ay = h - 20;
      ...
      ctx.beginPath(); ctx.arc(dx(Math.max(DMIN, Math.min(DMAX, dpi))), ay, 3, 0, Math.PI * 2); ctx.fill();
    };

    Fig.controls(el, [{ key: "dpi", label: "dpi", min: DMIN, max: DMAX, step: 1, value: target }],
      (k, v) => {
        // re-target the ease from the currently displayed value
        const p = Fig.reduced() ? 1 : Math.min(1, (performance.now() - anim.t0) / EASE_MS);
    ...
    Fig.caption(el, "solid = fits budget \u00B7 dashed = alibaba keeps \u00B7 blur = over-budget");
